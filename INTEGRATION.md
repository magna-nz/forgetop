...
**Unless the provider is required.** `FORGETOP_IT_PROVIDERS` is a comma-separated list of
providers that *must* run; one named there with absent credentials **fails** rather than
skipping. CI sets all six. This exists because a skip is otherwise indistinguishable from a
pass — the suite exits 0 either way — which is how Jira's missing credentials went unnoticed
for months while a removed Atlassian endpoint broke work-item listing for every user.
...
| Token | A **scoped** Atlassian API token (id.atlassian.com → API tokens → *with scopes* → app: Bitbucket) carrying `read:repository:bitbucket` (+ `read:user:bitbucket`). App passwords are gone. For using the app itself rather than this one read-only test, you also want `read`/`write:pullrequest:bitbucket` and `read`/`write:pipeline:bitbucket`. |
...
> Bitbucket now has CI secrets too. Note the credential shape changed: **app passwords are gone**,
> so `FORGETOP_IT_BITBUCKET_APP_PASSWORD` holds a *scoped* Atlassian API token (Bitbucket scopes),
> and `FORGETOP_IT_BITBUCKET_USERNAME` holds the account **email**, not the Bitbucket username —
> that is the position the token authenticates in. An unscoped token (the kind Jira accepts)
> returns 401 from the Bitbucket API.
...
Releases **hard-gate** on this suite. `dist-workspace.toml` sets
`plan-jobs = ["./integration"]`, so `dist init` generates a `custom-integration` job that
`build-local-artifacts` depends on — a red suite stops the release at plan time, before any
binary, installer or Homebrew formula is built. Don't hand-edit `release.yml`; change
`dist-workspace.toml` and re-run `dist init`.

The gate skips on pull requests (dist dry-runs the whole release pipeline on every PR, and
without that the live suite would run twice per PR against the same fixtures). It applies
where it matters: on the `v*` tag.

This was not always so — v0.11.0 shipped to Homebrew and both installers while Integration
was red, because `release.yml` is triggered by the tag independently of the other workflows.
