...
`release.yml` is autogenerated by cargo-dist and shouldn't be hand-edited, so the
integration suite gates releases via **branch protection**: make
`live provider integration` a **required status check** on `main` (Settings →
Branches). Since releases are tagged from `main`, released code has always passed the
suite; the workflow also runs on the `v*` tag itself as a visible check.
